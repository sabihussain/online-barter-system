                    ...
                    <div class="row text-center">
                        ...
                        <%
                            ...
                            String cat = request.getParameter("cat");
                                ResultSet r1 = DB.executeQuery("select * from product where pcat='" + cat + "' and subcat='"+sub
                                    +"' and pcode not in(select pcode from orders) and uid in(select id from register where dst=(select dst from register where id="+session.getAttribute("uid")+")) and uid!="+session.getAttribute("uid"));
                           if(r1.next()){
                            ResultSet rs = DB.executeQuery("select * from product where pcat='" + cat + "' and subcat='"+sub
                                    +"' and pcode not in(select pcode from orders) and uid in(select id from register where dst=(select dst from register where id="+session.getAttribute("uid")+")) and uid!="+session.getAttribute("uid"));
                            while (rs.next()) {
                        %>
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6 col-xxs-12">
                            <div style="background-color: #cc00cc">
                             <img src="uploads/<%=rs.getString("fpath")%>"/>
                                ...
                                <h2> <a href="exchange.jsp?id=<%=rs.getString("pcode")%>&cat=<%=cat%>" class="content-link"> EXCHANGE</a></h2>
